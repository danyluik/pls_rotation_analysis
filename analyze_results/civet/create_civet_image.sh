...
# Generate views
echo """ray_trace -output ${tmpdir}/left_medial.rgb ${tmpdir}/left_final.obj -size 400 400 -crop -sup 3 -bg white -right -shadows -directional 1 -1 -1 1 1 1 -directional -1 -1 -1 1 1 1 -directional -1 -1 1 1 1 1
ray_trace -output ${tmpdir}/left_lateral.rgb ${tmpdir}/left_final.obj -size 400 400 -crop -sup 3 -bg white -left -shadows -directional 1 -1 -1 1 1 1 -directional -1 -1 -1 1 1 1 -directional -1 -1 1 1 1 1
ray_trace -output ${tmpdir}/right_medial.rgb ${tmpdir}/right_final.obj -size 400 400 -crop -sup 3 -bg white -left -shadows -directional 1 -1 -1 1 1 1 -directional -1 -1 -1 1 1 1 -directional -1 -1 1 1 1 1
ray_trace -output ${tmpdir}/right_lateral.rgb ${tmpdir}/right_final.obj -size 400 400 -crop -sup 3 -bg white -right -shadows -directional 1 -1 -1 1 1 1 -directional -1 -1 -1 1 1 1 -directional -1 -1 1 1 1 1""" | parallel
...
if [[  ${#_arg_colourmap[@]} == 2 ]]; then
  if [[ -s ${_arg_colourmap[0]} ]]; then
    perl -e "${create_bar_graph}" -- -width 12 -height 120 -min 1 -max 0 -usercc ${_arg_colourmap[0]} ${tmpdir}/colourbar_neg.png
  else
    perl -e "${create_bar_graph}" -- -width 12 -height 120 -min 1 -max 0 -colour ${_arg_colourmap[0]} ${tmpdir}/colourbar_neg.png
  fi

  if [[ -s ${_arg_colourmap[1]} ]]; then
    perl -e "${create_bar_graph}" -- -width 12 -height 120 -usercc ${_arg_colourmap[1]} ${tmpdir}/colourbar_pos.png
  else
    perl -e "${create_bar_graph}" -- -width 12 -height 120 -colour ${_arg_colourmap[1]} ${tmpdir}/colourbar_pos.png
  fi

  convert ${tmpdir}/colourbar_pos.png ${tmpdir}/colourbar_neg.png -append ${tmpdir}/colourbar.png

  if [[ -n ${_arg_colourbar_labels} ]]; then
    colourbar_label_low=(-gravity West -annotate +60+60 "$(echo ${_arg_colourbar_labels} | cut -d, -f 1)")
    colourbar_label_high=(-gravity NorthWest -annotate +55+10 "+$(echo ${_arg_colourbar_labels} | cut -d, -f 2)")
    colourbar_label_high+=(-gravity SouthWest -annotate +55-107 "−$(echo ${_arg_colourbar_labels} | cut -d, -f 2)")
  fi
else
  if [[ -s ${_arg_colourmap[0]} ]]; then
    perl -e "${create_bar_graph}" -- -width 12 -height 240 -usercc ${_arg_colourmap[0]} ${tmpdir}/colourbar.png
  else
    perl -e "${create_bar_graph}" -- -width 12 -height 240 -colour ${_arg_colourmap[0]} ${tmpdir}/colourbar.png
  fi

  if [[ -n ${_arg_colourbar_labels} ]]; then
    colourbar_label_low=(-gravity SouthWest -annotate +55-107 "$(echo ${_arg_colourbar_labels} | cut -d, -f 1)")
    colourbar_label_high=(-gravity NorthWest -annotate +55+10 "$(echo ${_arg_colourbar_labels} | cut -d, -f 2)")
  fi
fi
...
convert -background white \
  -stroke white -strokewidth 2 -fill black -pointsize 22 \
...
if [[ ${_arg_draw_colourbar} == "on" ]]; then
  colourbar_draw="${tmpdir}/colourbar.png -bordercolor black -border 2x2 -bordercolor white -border 25x25 -gravity East -splice 140x0 "
  
  # Parameter expansion to get output directory
  # outfile=${_arg_output}
  # outdir="${outfile%/*}"
  convert -pointsize 22 \
    ...
    ${tmpdir}/colourbar.png

  convert -background white ${tmpdir}/final.mpc ${tmpdir}/colourbar.png -gravity Center +append ${tmpdir}/final.mpc
fi
